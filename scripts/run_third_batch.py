...
from mset.third_batch import build_phase3_conditions, build_phase3_design, iter_phase3_run_specs  # noqa: E402
...
def _run_spec(spec: dict[str, Any]) -> dict[str, Any]:
    ...
    try:
        env = make_environment(config, capture_events=False, trajectory_hashes=False)
        env.run()
...
def _audit_spec(spec: dict[str, Any]) -> dict[str, Any]:
    config = config_from_dict(spec["config"])
    left = make_environment(config, capture_events=False, trajectory_hashes=True)
    right = make_environment(config, capture_events=False, trajectory_hashes=True)
    left.run()
    right.run()
    return {
        "run_id": spec["run_id"],
        ...
        "learning_architecture": config.policy_mix[0],
        ...
        "ticks": left.tick,
        "state_hashes": left.state_hashes,
        "event_hash": left.event_hash(),
        "final_state_hash": left.final_state_hash(),
    }
...
def _write_exports(rows: list[dict[str, Any]], output_dir: Path) -> dict[str, Any]:
    ...
    return {
        "runs_csv_gz": str(csv_gz),
        "runs_csv_gz_sha256": hashlib.sha256(csv_gz.read_bytes()).hexdigest(),
        "rows": len(rows),
    }
...
def _audit_sample(specs: list[dict[str, Any]], sample_size: int) -> list[dict[str, Any]]:
    ...
    for spec in specs:
        config = spec["config"]
        key = (spec["family"], str(config["environment_variant"]), str(config["policy_mix"][0]))
        groups.setdefault(key, []).append(spec)
    selected: list[dict[str, Any]] = []
    per_group = max(1, sample_size // len(groups))
    for key in sorted(groups):
        rows = groups[key]
        count = min(per_group, len(rows))
    ...
    if len(selected) < min(sample_size, len(specs)):
        chosen = {row["run_id"] for row in selected}
        selected.extend(row for row in specs if row["run_id"] not in chosen and len(selected) < sample_size)
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Run the frozen MSET Phase III learning-agent validation.")
    parser.add_argument("--base", default="configs/confirmatory/phase3_core_validation_base.json")
    parser.add_argument("--output", default="results/phase3_core_validation")
    ...
    design = build_phase3_design(base_path)
    if not args.design_only:
        design["code_commit_at_execution"] = _git_commit()
    (output_dir / "design.json").write_text(json.dumps(design, ensure_ascii=False, indent=2, sort_keys=True) + "\n", encoding="utf-8")
    if args.design_only:
        print(json.dumps({key: design[key] for key in ("condition_count", "planned_runs", "planned_max_ticks", "design_hash")}, indent=2))
        return 0
    ...
    if args.family:
        ...
        conditions = [condition for condition in conditions if condition.family in allowed]
    specs = list(iter_phase3_run_specs(base, conditions, design["design_hash"]))
    ...
    with jsonl_path.open(mode, encoding="utf-8") as handle:
        with ProcessPoolExecutor(max_workers=max(1, args.workers)) as executor:
            for index, result in enumerate(executor.map(_run_spec, pending, chunksize=4), start=1):
    ...
    with ProcessPoolExecutor(max_workers=max(1, min(args.workers, 8))) as executor:
        audits = list(executor.map(_audit_spec, audit_specs, chunksize=1))
    audit_report = {
        ...
        "checks": [{key: value for key, value in row.items() if key not in {"config", "state_hashes"}} for row in audits],
    }
    (output_dir / "determinism_audit.json").write_text(json.dumps(audit_report, indent=2, sort_keys=True) + "\n", encoding="utf-8")
    replay_path = output_dir / "replay_bundles.json.gz"
    with gzip.open(replay_path, "wt", encoding="utf-8") as handle:
        json.dump({"design_hash": design["design_hash"], "bundles": audits}, handle, ensure_ascii=False, sort_keys=True)
    ...
    status = {
        ...
        "code_commit_at_execution": design.get("code_commit_at_execution", "not-recorded"),
        ...
        "replay_bundles": str(replay_path),
        "replay_bundles_sha256": replay_sha,
        **exports,
    }
    (output_dir / "status.json").write_text(json.dumps(status, indent=2, sort_keys=True) + "\n", encoding="utf-8")
